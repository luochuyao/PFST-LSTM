...
class ST_TrajLSTMCell(ConvRNNCell):
    def __init__(self, cell_param):
        super(ST_TrajLSTMCell, self).__init__(cell_param)
        self.L = cell_param['L']
    ...
    def cell(self, x_t, hidden,M):
        ...
        g = torch.tanh(
            ...
            F.conv2d(h2h_slice[0],self.w_hg,bias=None, padding=self.same_padding(self.state_to_state_kernel_size))+
            self.b_g
        )
        i = torch.sigmoid(
            F.conv2d(x_t,self.w_xi,bias=None, padding=self.same_padding(self.input_to_state_kernel_size))+
            F.conv2d(h2h_slice[1],self.w_hi,bias=None, padding=self.same_padding(self.state_to_state_kernel_size))+
            self.b_i
        )

        f = torch.sigmoid(
            F.conv2d(x_t,self.w_xf,bias=None, padding=self.same_padding(self.input_to_state_kernel_size))+
            F.conv2d(h2h_slice[2],self.w_hf,bias=None, padding=self.same_padding(self.state_to_state_kernel_size))+
            self.b_f
        )
        c = f*c_tm1 + i*g
