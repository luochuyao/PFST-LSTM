...
class sequence_model(object):
    def __init__(self,name,encoder_decoder_model,info):
        self.test_save_root = '/mnt/A/meteorological/2500_ref_seq/CIKM_dec_ST_TrajLSTM_test/'
        self.validation_save_root = '/mnt/A/meteorological/2500_ref_seq/CIKM_dec_ST_TrajLSTM_validation/'
        self.name = name
...
if __name__ == '__main__':
    path = 'config/dec_ST_TrajLSTM_CIKM.yml'
    f = open(path)
    ...
    for idx, cell in enumerate(configuration['MODEL_NETS']['ENCODE_CELLS']):
        ...
        param['L'] = configuration['MODEL_NETS']['Ls'][idx]
    ...
    encoder_decoder_model = Encode_Decode_ST_TrajLSTM(
        encoder=encoder,
        decoder=decoder,
        info=configuration
    ).cuda()
